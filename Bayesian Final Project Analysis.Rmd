...
p=4
X_BASEL = X_new[1:731, 1:3]
y_BASEL = y_new[1:731, 1]
...
proposal_alpha_beta <- function(alpha.beta, var.prop) {
...
ALPHA.BETA.BASEL <- matrix(0,nrow=S/R,ncol=p)
ALPHA.BETA.BASEL[1,]=alpha.beta
...
alpha.beta.star <- proposal_alpha_beta(alpha.beta, var.prop)
...
if (s %% R == 0) { 
  print(s)
  ALPHA.BETA.BASEL[s/R,] <- alpha.beta}
}
ACR_BASEL <- c(ACR_BASEL,ac/S)
ACF_BASEL <- c(ACF_BASEL,acf(ALPHA.BETA.BASEL[,1],plot=FALSE)$acf[2])
```
```{r}
ACR_BASEL
```
...
#ACF Plots
par(mfrow = c(1, 4)) # Set up the layout for side-by-side plots
acf(ALPHA.BETA.BASEL[,1])
acf(ALPHA.BETA.BASEL[,2])
acf(ALPHA.BETA.BASEL[,3])
acf(ALPHA.BETA.BASEL[,4])
```
...
#1: Do Logistic Regression on BASEL
p=4
X_BUDAPEST = X_new[1:731, 4:6]
y_BUDAPEST = y_new[1:731, 2]
...
alpha.beta <- rep(0, p) #initializing the markov chain
#alpha.beta = c(-30, -7, -30, 1)
ALPHA.BETA.BUDAPEST <- matrix(0,nrow=S/R,ncol=p)
ALPHA.BETA.BUDAPEST[1,]=alpha.beta
var.prop <- diag(delta2, nrow = p, ncol = p)
set.seed(1)
...
if (s %% R == 0) { 
  print(s)
  ALPHA.BETA.BUDAPEST[s/R,] <- alpha.beta}
}
ACR_BUDAPEST <- c(ACR_BUDAPEST,ac/S)
ACF_BUDAPEST <- c(ACF,acf(ALPHA.BETA.BUDAPEST[,1],plot=FALSE)$acf[2])
```
...
library(coda)
print("Effective sample size for alphaˆ(s) and betaˆ(s)")
apply(ALPHA.BETA.BUDAPEST,2,effectiveSize)
```

```{r}
print("Posterior mean and variances of alpha, beta")
apply(ALPHA.BETA.BUDAPEST,2,mean)
```

```{r}
apply(ALPHA.BETA.BUDAPEST,2,var)
```

```{r}
par(mfrow = c(1, 4)) # Set up the layout for side-by-side plots
# Plot for the first row (ALPHA)
density_alpha <- density(ALPHA.BETA.BUDAPEST[,1])
plot(density_alpha, col = "blue", main = "BUDAPEST ALPHA",xlim = c(-5,5))
lines(density(rnorm(S, 0, 10)), col = "red") # Add a reference line
legend("topright", legend = c("Posterior", "Prior"), col = c("blue", "red"), lty = 1)

# Plot for the second row (BETA 1)
density_beta_1 <- density(ALPHA.BETA.BUDAPEST[,2])
plot(density_beta_1, col = "blue", main = "BUDAPEST BETA Humidity",xlim = c(-5,5))
lines(density(rnorm(S, 0, 10)), col = "red") # Add a reference line
legend("topright", legend = c("Posterior", "Prior"), col = c("blue", "red"), lty = 1)

# Plot for the third row (BETA 2)
density_beta_2 <- density(ALPHA.BETA.BUDAPEST[,3])
plot(density_beta_2, col = "blue", main = "BUDAPEST BETA Precipitation",xlim = c(-5,5))
lines(density(rnorm(S, 0, 10)), col = "red") # Add a reference line
legend("topright", legend = c("Posterior", "Prior"), col = c("blue", "red"), lty = 1)

# Plot for the fourth row (BETA 3)
density_beta_3 <- density(ALPHA.BETA.BUDAPEST[,4])
plot(density_beta_3, col = "blue", main = "BUDAPEST BETA Mean Temperature",xlim = c(-5,5))
lines(density(rnorm(S, 0, 10)), col = "red") # Add a reference line
legend("topright", legend = c("Posterior", "Prior"), col = c("blue", "red"), lty = 1)
```
...
#ACF Plots
par(mfrow = c(1, 4)) # Set up the layout for side-by-side plots
acf(ALPHA.BETA.BUDAPEST[,1])
acf(ALPHA.BETA.BUDAPEST[,2])
acf(ALPHA.BETA.BUDAPEST[,3])
acf(ALPHA.BETA.BUDAPEST[,4])
```
...
```{r}
#try varying delta and S for BASEL
ACR_BASEL<-NULL
ACF_BASEL<-NULL
...
for(delta2 in 2^c(-7, -6, -5, -4, -3)){ #this delta gave us about 18% of the acceptance rate
#variance-covariance matrix of a proposal distribution
var.prop <- diag(delta2, nrow = p, ncol = p)
...
ACR_BASEL <- c(ACR_BASEL,ac/S)
ACF_BASEL <- c(ACF_BASEL,acf(ALPHA.BETA.BASEL[,2],plot=FALSE)$acf[2])
}

```

```{r}
plot(ACR_BASEL,ACF_BASEL, main="ACF (lag 1) vs ACR for humidity in BASEL") ; lines(ACR_BASEL,ACF_BASEL)
```
