...
for (i in 1:45) {
  if ((i - 1) %% 3 == 0) {
    ...
    model = glm(y_to_consider~., data=X_to_consider, family="binomial", maxit=1000)
...
prior_alpha_beta <- function(alpha.beta) {
  return(dnorm(alpha.beta, rep(0, p), rep(10, p), log=T))  
}
...
log_likelihood <- function(y, alpha.beta, x) {
  p_y_given_theta = y*(alpha.beta[1]+alpha.beta[2:p]*x)-log(1+exp(alpha.beta[1]+alpha.beta[2:p]*x))
...
print(logr)
if( log(runif(1)) < logr ) { alpha.beta <- alpha.beta.star; ac <- ac+1 }
if (s %% R == 0) { ALPHA.BETA[s/R,] <- alpha.beta}
}
ACR <- c(ACR,ac/S)
